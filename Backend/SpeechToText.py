...
from selenium.webdriver.common.by import By  # Import the By class here
...
# Specify the service using the Service class
...
def QueryModifier(Query):
    ...
    # Check if the query is a question and add a question mark if needed.
    if query_words[0] in question_words:
        if query_words[-1][-1] in ['.', '?', '!']:
            new_query = new_query[:-1] + "?"
        else:
            new_query += "?"
    else: # Add a period if the query is not a question.
        if query_words[-1][-1] in ['.', '?', '!']:
            new_query = new_query[:-1] + "."
        else:
            new_query += "."
        ...
        try:
            ...
            if Text:
                ...
                if InputLanguage.lower() == "en" or "en" in InputLanguage.lower():
                    ...
                    # If the input language is not English, translate the
